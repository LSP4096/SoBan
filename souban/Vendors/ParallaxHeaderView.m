...
+ (id)parallaxHeaderViewWithImage:(UIImage *)image avatar:(NSString *)avatar forSize:(CGSize)headerSize
{
    ParallaxHeaderView *headerView = [[ParallaxHeaderView alloc] initWithFrame:CGRectMake(0, 0, headerSize.width, headerSize.height)];
    headerView.headerImage = image;
    [headerView initialSetupForDefaultHeader];
    UIImageView *imageView = [[UIImageView alloc] init];
    imageView.image = [UIImage imageNamed:@"ic_user_placeholder"];
    imageView.frame = CGRectMake(0, 0, 80 * headerSize.width / 375, 80 * headerSize.width / 375);
    imageView.center = CGPointMake(headerSize.width / 2, headerSize.height / 2);
