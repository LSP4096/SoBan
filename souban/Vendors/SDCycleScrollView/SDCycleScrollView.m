...
- (void)initialization
{
    ...
    _mainView.decelerationRate = UIScrollViewDecelerationRateNormal;
}
...
- (void)loadImageAtIndex:(NSInteger)index
{
    ...
    if (data) {
        ...
        [[SDWebImageManager sharedManager].imageCache imageFromDiskCacheForKey:url.absoluteString];
...
- (void)setupPageControl
{
    ...
    pageControl.dotImage = [[UIImage imageWithSolidColor:[UIColor colorWithHex:0xffffff alpha:0.9] size:CGSizeMake(7, 7)] makeRoundedWithRadius:3.5];
    pageControl.currentDotImage = [[UIImage imageWithSolidColor:[UIColor colorWithHex:0x0682ff] size:CGSizeMake(7, 7)] makeRoundedWithRadius:3.5];
...
{
    if (1 >= _imagesGroup.count) return;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:didSelectItemAtIndex:imageView:)]) {
        [self.delegate cycleScrollView:self didSelectItemAtIndex:indexPath.item % self.imagesGroup.count imageView:cell.imageView];
    }
}
...
- (UIImageView *)placeHolderImageView
{
    if (!_placeHolderImageView) {
        _placeHolderImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"placeholder_aptmt"]];
